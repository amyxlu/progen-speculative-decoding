...
for i in {1..10}; do
    for ((ntoks=1; ntoks<10; ntoks+=1)); do
        MODEL=progen2-xlarge SPEC_MODEL=progen2-small NUM_SPEC_TOKENS=$ntoks NUM_SAMPLES=4 MAX_LENGTH=64 BENCHMARK=False LOG_TO_WANDB=True SAMPLE=True LOG_SPEC_DECODE_METRICS=False ./run_sample.sh
    done
    sleep 10 
done
